...
Maizal.prototype.setup = function(config) {
  ...
  this._pool = definePool(config.initial, config.engine.evaluationFn, this._goals)
...
Maizal.prototype.expand =  function expand() {
  ...
  if (!_state) return Promise.reject('The goal states could not be reached. Are you sure they are accesible?')
  ...
  let promises = this._actions.map(action => {
    return new Promise((resolve) => {
      ...
      return states.filter(state => (state && !this._closed.has(state)))
    }).then(states => {
 ...
 */
function definePool(initial, evaluationFn, goals) {
